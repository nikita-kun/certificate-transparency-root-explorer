...
function X509() {
    ...
    this.getVersion = function() {
		...
		if (_getTLVbyList(this.hex, 0, [0, 0]) !==
			...
			if (_getTLVbyList(this.hex, 0, [0, 0]) !== "a003020100")
				this.foffset = -1;
			else 
				this.foffset = 0;
			return 1;
	...
	if (hKeyUsage.length % 2 != 0 || hKeyUsage.length <= 2)
	    return "?" //throw "malformed key usage value";
	...
	for (var i = 0; i < a.length; i++) {
		...
		for (var j = 0; j < a2.length; j++) {
			...
			try {
				...
				policyInfo.cps = "failed to decode";
				continue;
			}
...
X509.hex2dn = function(hex, idx) {
    ...
    if (hex.substr(idx, 2) !== "30") return ""; //throw "malformed DN";
...
X509.hex2rdn = function(hex, idx) {
    if (idx === undefined) idx = 0;
    if (hex.substr(idx, 2) !== "31") return "";//throw "malformed RDN";
